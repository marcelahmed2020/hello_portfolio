@section('site_name',' | '.__('site.edit').' / '. $service->title)
    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> @lang('site.edit') {{ ' / '. $service->title  }}</li>
            </ol>
        </nav>
             <form action="{{ route('admin.services.update',$service->id) }}" id="form_validation" method="post" enctype="multipart/form-data">
                                ...
                                @foreach (config('translatable.locales') as  $locale)
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text"  class="form-control" placeholder="@lang('site.'.$locale.'.title')" value="{{! empty($service->translate($locale)->title)?$service->translate($locale)->title:''}}" name="{{$locale}}[title]" id="{{$locale.'.name'}}">
                                ...
                                @foreach (config('translatable.locales') as  $locale)
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text"  class="form-control" placeholder="@lang('site.'.$locale.'.desc')" value="{{! empty($service->translate($locale)->desc)?$service->translate($locale)->desc:''}}" name="{{$locale}}[desc]" id="{{$locale.'.desc'}}">
                                        </div>
                                    </div>
                                @endforeach
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text"  class="form-control" placeholder="@lang('site.icon')" value="{{ $service->icon   }}" name="icon" id="icon">
